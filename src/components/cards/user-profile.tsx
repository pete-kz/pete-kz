...
const ChangeProfileForm = lazy(
  () => import("@/components/forms/change-profile"),
)
...
export default function UserProfileCard({ user }: { user: User_Response }) {
  ...
  const userLastUpdated = useMemo(() => {
    ...
    return parsedDate
      ? `${parsedDate.date.day}/${parsedDate.date.month}/${parsedDate.date.year} ${parsedDate.time.hour}:${parsedDate.time.minutes}`
      : ""
  }, [user])
    ...
    <Card className="p-3 flex flex-col gap-4">
      ...
      {authState && authState?._id === user._id && (
        <div className="grid grid-cols-2 border grid-rows-1 rounded-lg">
          <ChangeProfileForm>
            <Button
              className="rounded-none p-2 m-0 border-r gap-2 rounded-l-lg"
              type="submit"
              variant={"link"}
            >
              {t("label.edit")}
