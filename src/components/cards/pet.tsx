...
export default function PetCard(props: PetCard) {
  ...
  return (
    <>
      <PetOverlay pet={props} owner={props.user} info open={openPet} setOpen={setOpenPet} />
      <div className='relative cursor-pointer' style={{ aspectRatio: '3/4', overflow: 'hidden', minWidth: '100%' }} onClick={() => { setOpenPet(true) }}>
        <img className='rounded-lg' src={props.imagesPath[0]} alt={props.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        <div className='rounded-lg absolute flex items-end bottom-0 left-0 p-3 bg-gradient-to-t w-full h-full from-black from-2% to-transparent to-40%'>
          <div>
            ...
            <p>{formatAge(props.birthDate, t('pet.year'), t('pet.month'))}</p>
          </div>
        </div>
      </div>
    </>
