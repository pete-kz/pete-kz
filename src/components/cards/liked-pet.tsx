...
export default function LikedPet({
  pet_id,
  userData,
  setUserLiked,
}: {
  pet_id: Pet_Response["_id"]
  userData: User_Response | undefined
  setUserLiked: React.Dispatch<React.SetStateAction<Pet_Response[]>>
}) {
  // States
  ...
  const {
    ...
    queryFn: () =>
      axios.get(`${API.baseURL}/pets/find/${pet_id}`).then((res) => res.data),
  ...
  function removePetFromLiked(pet_id: string) {
    ...
    if (!isAuthenticated() || !userData) {
      ...
      let browserLiked = JSON.parse(
        localStorage.getItem("_data_offline_liked") || "[]",
      ) as string[]
  ...
  return (
    <Card className="flex items-center justify-between mt-2 p-3 cursor-pointer">
      {pet && (
        <>
          {open && (
            <PetOverlay open setOpen={setOpen} pet={pet} info contacts />
          )}
          <div
            className="w-full"
            onClick={() => {
