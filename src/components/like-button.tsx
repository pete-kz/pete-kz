...
export default function LikeButton(props: { pet: Pet_Response }) {
    ...
    return (
        <div className='sticky w-full bottom-5 flex justify-center'>
            <Button variant={'outline'} size={'icon'} className='w-fit p-6 rounded-full gap-2' style={{ color: '#FF0000' }} onClick={likePet}><span className='text-white'>{t('label.addToLiked')}</span><Heart fill={liked ? '#FF0000' : 'transparent'} /></Button>
        </div>
    )
}
