...
export function formatAge(
  ...
  const monthsString: string =
    Number(months) > 0 ? `${months} ${i18_months}` : ""
...
export function axiosErrorHandler(error: AxiosError) {
  if (error.response) {
    if (error.response.status === 500)
      return toast({
        description: i18n.t(
          `api.${(error.response.data as { msg: APIErrors }).msg}`,
        ),
        variant: "destructive",
      })
...
function parseMongoDate(Mongo_Date: string) {
  ...
  if (Mongo_Date) {
    ...
    const date = {
      ...
      month:
        Mongo_Date.split("-")[1].split("T")[0][0] != "0"
          ? Mongo_Date.split("-")[1].split("T")[0]
          : Mongo_Date.split("-")[1].split("T")[0][1],
      day:
        Mongo_Date.split("-")[2].split("T")[0][0] != "0"
          ? Mongo_Date.split("-")[2].split("T")[0]
          : Mongo_Date.split("-")[2].split("T")[0][1],
    }

    const time = {
      hour:
        parsedTime.split(":")[0][0] != "0"
          ? Number(parsedTime.split(":")[0])
          : Number(parsedTime.split(":")[0][1]),
      minutes:
        parsedTime.split(":")[1][0] != "0"
          ? Number(parsedTime.split(":")[1])
          : Number(parsedTime.split(":")[1][1]),
    }
