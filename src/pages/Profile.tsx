...
export default function Profile() {
  ...
  const {
    ...
    queryKey: ["pets"],
    queryFn: () =>
      axios.get(`${API.baseURL}/pets/find`).then((res) => res.data),
  ...
  useEffect(() => {
    if (petsData) {
      ...
      setUserPets(
        (petsData as Pet_Response[]).filter((pet) => pet.ownerID === user?._id),
      )
      setUserLiked(
        (petsData as Pet_Response[]).filter((pet) =>
          likedIds.includes(pet._id),
        ),
      )
    }
  }, [petsData, userData])
...
function SettingsButton() {
  ...
  return (
    <button
      className="p-3 bg-primary rounded-full text-border"
