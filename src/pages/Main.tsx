...
export default function Main() {
  ...
  function savePetsToLocal(pets: Pet_Response[]): void {
    localStorage.setItem(
      "_data_allPets",
      JSON.stringify(allPets.length < 20 ? pets : allPets),
    )
    setAllPets(pets)
  }
      ...
      <div className="p-4 flex flex-col items-center w-full justify-center h-screen">
        <div className="max-w-md">
          {loadingPets ? (
            ...
            <>
              {updatingCache && !loadingPets && (
                <p className="animate-pulse font-semibold bg-card p-4 mb-2 rounded-lg border w-full">
                  {t("label.updatePets")}...
                </p>
              )}
              ...
              {typeof allUsers.filter === "function" && (
                <div className="flex w-full gap-2 justify-center px-3 mt-2">
                  ...
                  <Button
                    ...
                    className="active:scale-95"
                    disabled={
                      allPets[current]._id === allPets[allPets.length - 1]._id
                    }
                    onClick={() => {
                      api?.scrollNext()
                    }}
