...
export default function User() {
    ...
    const { data: user, error, isPending } = useQuery<User_Response, AxiosError>({ queryKey: ['user', userId], queryFn: () => axios.get(`${API.baseURL}/users/find/${userId}`).then(res => res.data) })
    const { data: pets } = useQuery<Pet_Response[], AxiosError>({
        queryKey: ['user_pets', userId], queryFn: () => axios.get(`${API.baseURL}/pets/find/`).then(res => {
