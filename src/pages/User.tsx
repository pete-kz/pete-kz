...
export default function User() {
  ...
  const {
    ...
    queryFn: () =>
      axios.get(`${API.baseURL}/users/find/${userId}`).then((res) => res.data),
  })
  const { data: pets } = useQuery<Pet_Response[], AxiosError>({
    queryKey: ["user_pets", userId],
    queryFn: () =>
      axios.get(`${API.baseURL}/pets/find/`).then((res) => {
        return (res.data as Pet_Response[]).filter(
          (pet) => pet.ownerID === user?._id,
        )
      }),
  })
  ...
  return (
    <>
      <MobilePageHeader
        to="/pwa"
        title={
          (user?.companyName ? user?.companyName : user?.firstName) || "404"
        }
      />
